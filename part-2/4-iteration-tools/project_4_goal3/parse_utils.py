...
def iter_file(fname, class_name, parser):
    ...
    for row in reader:
        yield nt_class(*(parse(value) for value, parse in zip(row, parser)))
...
def filtered_iter_combined(fname, class_names, parsers, compress_fields, *, key=None):
    iter_combo = iter_combined(fname, class_names, parsers, compress_fields)
